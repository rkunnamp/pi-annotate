...
export type ToolbarProps = {
  // Controlled state (managed by parent App.tsx)
  annotations: Annotation[];
  ...
  // UI options
  showChatOption?: boolean;  // Show "Send & Chat" button
  
  // Demo mode (preserved from original)
...
export function Toolbar({
  ...
  // Toolbar is always active when rendered (parent controls visibility)
  ...
  useEffect(() => {
    ...
    // Only load from localStorage if parent hasn't already set annotations
    // This allows the parent to control whether to restore or start fresh
    ...
    // Note: settings are now controlled by parent, but we can still merge localStorage preferences
    try {
      const storedSettings = localStorage.getItem("feedback-toolbar-settings");
      if (storedSettings) {
        setSettings(prev => ({ ...DEFAULT_TOOLBAR_SETTINGS, ...prev, ...JSON.parse(storedSettings) }));
      }
    } catch (e) {
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pathname]); // Note: annotations.length check is intentionally not in deps - we only want this to run on mount
  ...
  // Send annotations to Pi (replaces copy-to-clipboard)
  const handleSendToPi = useCallback((withChat?: boolean) => {
    if (annotations.length === 0) return;
    
    if (withChat) {
      // Show a simple prompt for the message (or could skip and let parent handle)
      const message = window.prompt("Add a message for Pi:", "");
      if (message === null) {
        // User cancelled - don't send anything
        return;
      }
      // Show feedback animation and send
      setCopied(true);
  ...
  }, [annotations.length, onSend, settings.autoClearAfterCopy, clearAll]);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        if (pendingAnnotation) {
          ...
          // Close toolbar entirely (notify parent)
          onClose();
        }
      }
    };
    ...
    document.addEventListener("keydown", handleKeyDown);
    return () => document.removeEventListener("keydown", handleKeyDown);
  }, [isActive, pendingAnnotation, onClose]);
        ...
        >
          ...
          >
            <div className={styles.settingsHeader}>
              ...
              <span className={styles.settingsVersion}>v0.1.0</span>
              <button
